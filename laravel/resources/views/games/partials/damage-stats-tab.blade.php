...
@else
    <ul class="nav nav-pills mb-3" id="damage-stats-subtabs" role="tablist">
        ...
        @foreach ($queryStats as $stat)
            <li class="nav-item" role="presentation">
                <button class="nav-link" id="damage-stats-query-{{ $stat['query']->idquery }}-tab" data-bs-toggle="pill" data-bs-target="#damage-stats-query-{{ $stat['query']->idquery }}-pane" type="button" role="tab" aria-controls="damage-stats-query-{{ $stat['query']->idquery }}-pane" aria-selected="false">{{ $stat['query']->label }}</button>
            </li>
        @endforeach
    </ul>
    ...
    <div class="tab-content" id="damage-stats-subtabs-content">
        ...
        @foreach ($queryStats as $stat)
            <div class="tab-pane fade" id="damage-stats-query-{{ $stat['query']->idquery }}-pane" role="tabpanel" aria-labelledby="damage-stats-query-{{ $stat['query']->idquery }}-tab">
                <div class="row g-3 mb-3">
                    <div class="col-md-6">
                        <div class="card bg-dark text-white h-100">
                            <div class="card-body">
