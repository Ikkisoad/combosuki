...
<x-layouts.app :title="'Edit Combo - '.$game->name">
            ...
            <div class="input-group mb-3">
                ...
                <select name="listingtype" class="form-select" required>
                    @foreach ($listingTypes as $entry)
                        <option value="{{ $entry->entryid }}" @selected(old('listingtype', $combo->type) == $entry->entryid)>{{ $entry->title }}</option>
                    @endforeach
                </select>
            </div>
                                ...
                                <div class="input-group mb-3 flex-nowrap">
                                    ...
                                    <select name="resources[{{ $resource->idgame_resources }}]" class="form-select resource-value-select" required>
                                        @foreach ($resource->values->sortBy([['order', 'asc'], ['value', 'asc']]) as $value)
                                            <option value="{{ $value->idResources_values }}" @selected(($selectedResources[$resource->idgame_resources]['value_id'] ?? null) == $value->idResources_values) data-default-label="{{ $value->value }}">{{ $value->value }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            @elseif ($resource->type === 3)
                                <div class="input-group mb-3 flex-nowrap">
                                    ...
                                    @for ($slot = 0; $slot < 2; $slot++)
                                        <select name="resources[{{ $resource->idgame_resources }}][]" class="form-select resource-value-select" required>
                                            @foreach ($resource->values->sortBy([['order', 'asc'], ['value', 'asc']]) as $value)
                                                <option value="{{ $value->idResources_values }}" @selected((($selectedResources[$resource->idgame_resources]['value_ids'] ?? [])[$slot] ?? null) == $value->idResources_values) data-default-label="{{ $value->value }}">{{ $value->value }}</option>
                                            @endforeach
                                        </select>
                                    @endfor
                                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            initSecondaryResources({{ $forceShowSecondaryResources ? 'true' : 'false' }});
