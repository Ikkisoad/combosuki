...
// route()-generated by the Blade shell, not hardcoded here: these live at a
// dedicated subdomain's root in production but under an "/activity/comble"
// prefix on the main domain in local dev — see routes/activity.php.
const urls = JSON.parse(document.getElementById('activity-comble-urls')?.textContent || '{}');
...
/** Attaches the Bearer token every Activity API call (besides the token exchange itself) needs — see VerifyActivityToken. */
...
async function loadState() {
    const response = await authorizedFetch(urls.state, { headers: { Accept: 'application/json' } });
