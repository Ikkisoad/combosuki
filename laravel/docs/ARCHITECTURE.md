...
- **Daily Challenge** (`App\Services\DailyChallenge`, `ChallengeController`, table `daily_challenge_picks`): each calendar day (in `DailyGameClock`'s timezone) gets a `(CharacterQuery, Character)` pair.
  ...
  - **In-place day browsing**: `/challenge/{date}` renders the day section from `challenge/partials/day`; `/challenge/{date}/panel` (`ChallengeController::dayPanel()`) returns just that partial, which `challenge-day.js` fetches (cached per date for the page's lifetime) to swap days on prev/next, date-picker, and calendar-cell clicks, syncing the URL via `history.pushState`. Links keep real hrefs, so a failed fetch falls back to a full page load. Any new link to another day on that page should carry `data-challenge-day="YYYY-MM-DD"` to get the same treatment.
  - **Calendar game filter**: `ChallengeController::calendarTab()` returns, besides each day's status, `day_games` (date → game id of that day's `CharacterQuery`, only for days with a challenge) and `games` (the year's games, `{id, name}`, sorted by name). `challenge-calendar.js` filters client-side — a selected game dims and un-links every other game's days, without re-fetching. The cached value holds game ids only; `games` is hydrated after the cache read (see §7's serialization gotcha).
...
- **Comble** (`App\Services\Comble*`, `CombleController`, tables `comble_daily_picks`/`comble_attempts`/`comble_day_views`): a separate Wordle-style "guess the day's combo" puzzle, also exposed as a Discord Activity. `CombleGuessEvaluator::starterMatch()` compares only the **first 6 characters** of the combo string with spaces stripped from both sides first — confirming combo-notation spacing (§8) is a display convention a player isn't expected to reproduce exactly.
  ...
  - **Video embeds inside the Activity must go through Discord's proxy**: Discord Activities run behind a proxy that blocks any request — including an iframe's own `src` — to a domain not registered as a "URL Mapping" in the Discord Developer Portal (app > Activities > URL Mappings); an unmapped request fails client-side with `blocked:csp` before it reaches the network. `<x-video-embed :video="..." activity />` (only passed `activity` from `activity/_comble-game.blade.php`, never from `comble/_game.blade.php`) rewrites the iframe/video `src` for the single-host providers (YouTube, Streamable, Twitch clips, Gfycat, Medal) through `App\Support\ActivityProxyUrl::rewrite()` into `/.proxy/<prefix>/...`. Each prefix there must have a matching Developer Portal entry (`youtube` → `www.youtube.com`, `streamable` → `streamable.com`, `twitch-clips` → `clips.twitch.tv`, `gfycat` → `gfycat.com`, `medal` → `medal.tv`) or the rewritten `src` 404s against Discord's proxy instead. The script-widget providers (Twitter/X, Imgur, NicoNico) fetch further cross-origin resources at runtime that no fixed mapping list can cover, so inside the Activity they render a plain "watch on X" link instead of the widget; the `file` provider (an arbitrary user-submitted host) is left unproxied for the same reason and stays best-effort inside the Activity.
